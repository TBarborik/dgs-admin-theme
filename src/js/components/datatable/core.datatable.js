...
(function ($) {
	...
	$.fn[pluginName] = function (options) {
		...
		var Plugin = {
			...
			afterRender: function (e, args) {
				...
				const tooltips = datatable.tableBody.querySelectorAll("[data-toggle='tooltip']")
			...
			adjustCellsWidth: function () {
				...
				var columns = $(datatable.tableHead).find('.' + pfx + 'datatable-row:first-child').find('.' + pfx + 'datatable-cell').
					// exclude expand icon
					not('.' + pfx + 'datatable-toggle-detail').not(':hidden')

				if (columns.length > 0) {
					//  remove reserved sort icon width
					containerWidth = containerWidth - (Plugin.iconOffset * columns.length)
					const ordinaryColumns = columns.not(".datatable-cell-check").length
					const checkColumns = columns.filter(".datatable-cell-check").length
					var minWidth = Math.floor(containerWidth / ordinaryColumns - 22 * checkColumns)
					...
					$(datatable.table).find('.' + pfx + 'datatable-row').find('.' + pfx + 'datatable-cell').
						...
						var width = td.classList.contains("datatable-cell-check") ? 22 : minWidth
			...
			getTablePrefix: function (suffix) {
				...
				suffix += window.location.pathname.replaceAll("/", "-")
	...
	$.fn[pluginName].defaults = {
		...
		layout: {
			...
			// datatable UI icons
			icons: {
				sort: {asc: 'fa-solid fa-arrow-up', desc: 'fa-solid fa-arrow-down'},
				pagination: {
					next: 'fa-solid fa-angle-right',
					prev: 'fa-solid fa-angle-left',
					first: 'fa-solid fa-angles-right',
				...
				pagination: {
					items: {
						default: {
							...
							more: 'Více stránek',
							input: 'Číslo stránky',
							select: 'Vyberte velikost stránky',
							all: 'Vše',
						},
						info: 'Zobrazuji {{start}} - {{end}} z {{total}}',
					},
				},
			},
		},
